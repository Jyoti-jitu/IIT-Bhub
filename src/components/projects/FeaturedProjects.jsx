...
const FeaturedCard = ({ project, onSelect, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative group cursor-pointer bg-slate-900 border ${border} rounded-3xl overflow-hidden transition-all duration-300`}
    >
      {/* BG glow */}
      ...
      <div className="relative z-10 p-7 md:p-8 flex flex-col h-full">
        ...
        {/* Type tag */}
        <div className="mb-3">
          <span className="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md border text-slate-400 bg-slate-800 border-slate-700">
            {project.type}
          </span>
          {project.openSource && (
        ...
        {/* Title */}
        <h3 className={`text-2xl md:text-3xl font-extrabold text-white mb-3 leading-snug group-hover:${accent} transition-colors`}>
          {project.title}
        </h3>
        ...
        {project.metrics && (
          <div className="grid grid-cols-2 gap-3 mb-6">
            {project.metrics.slice(0, 4).map((m) => (
              <div key={m.label} className="p-3 bg-slate-800/60 border border-slate-700/40 rounded-xl">
        ...
        <div className="flex flex-wrap gap-1.5 mb-6">
          {project.tech.map((t) => (
            <span key={t} className="px-2.5 py-1 text-[10px] font-semibold text-slate-400 bg-slate-800 border border-slate-700/50 rounded-md">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-auto">
          {project.github && (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-bold rounded-xl border border-slate-700 transition-all"
            >
  ...
  <section className="py-16 bg-slate-950 border-b border-slate-900">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-2xl font-extrabold text-white">Featured Projects</h2>
